...
namespace View { 
    public class DrawingPanel : Panel {
        ...
        private void PowerupDrawer(object o, PaintEventArgs e) {
            ...
            using (System.Drawing.Pen bluePen = new System.Drawing.Pen(System.Drawing.Color.Blue))
            using (System.Drawing.Pen greenPen = new System.Drawing.Pen(System.Drawing.Color.Green))
            using (System.Drawing.SolidBrush clearBrush = new System.Drawing.SolidBrush(System.Drawing.Color.Blue)) {
                // Circles are drawn starting from the top-left corner.
                // So if we want the circle centered on the powerup's location, we have to offset it
                // by half its size to the left (-width/2) and up (-height/2)
                Rectangle r = new Rectangle(-(width / 2), -(height / 2), width, height);
                Rectangle r2 = new Rectangle(-(width / 2) + 2, -(height / 2) + 2, width - 4, height - 4);

                e.Graphics.FillEllipse(redBrush, r);
                //e.Graphics.FillEllipse(clearBrush, r2);
                //if (p.GetKind() == 1) { // red powerup
                //    //e.Graphics.FillEllipse(redBrush, r);
                //    //e.Graphics.FillEllipse(clearBrush, r2);
                //    e.Graphics.DrawEllipse(redPen, r);
                //}
                //if (p.GetKind() == 2) { // red powerup
                //    //e.Graphics.FillEllipse(yellowBrush, r);
                //    //e.Graphics.FillEllipse(clearBrush, r2);
                //    e.Graphics.DrawEllipse(bluePen, r);
                //}
                //if (p.GetKind() == 1) { // red powerup
                //    //e.Graphics.FillEllipse(blackBrush, r);
                //    //e.Graphics.FillEllipse(clearBrush, r2);
                //    e.Graphics.DrawEllipse(greenPen, r);
                //}
            }
        }
